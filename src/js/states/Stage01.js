...
(function(){
    ...
    GameCtrl.Stage01.prototype = {
        ...
        create: function () {
            ...
            this.backgroundMusic = this.add.audio('background_music', 0.5, true);
            this.backgroundMusic.play();
            
            // Add sound effects
            this.jumpSound = this.add.audio('jump_sound');
            this.powerUpSound = this.add.audio('powerup_sound');
            this.collisionSound = this.add.audio('collision_sound');
            
            // Create score text
            this.scoreText = this.add.text(20, 20, 'Score: 0', {
                font: '32px "arcadeclasic"',
                fill: '#fff'
            });
        ...
        _createPowerUps: function() {
            var w = this.world.bounds.width - 800;
            for (var i = 1500; i < w; i += 1200) {
                var powerUp = this.powerUps.create(i, 400, 'clown', 'powerup0000');
                powerUp.scale.setTo(2);
        ...
        collectPowerUp: function(lion, powerUp) {
            powerUp.kill();
        ...
        update: function () {
            ...
            this.physics.arcade.overlap(this.lion, this.powerUps, this.collectPowerUp, null, this);

            // Update score based on distance
            this.score += Math.floor(this.lion.body.velocity.x * 0.01);
            this.scoreText.text = 'Score: ' + this.score;

            // Increase difficulty based on score
            if (this.score > this.difficultyLevel * 1000) {
                this.difficultyLevel++;
                this.levelText.text = 'Level: ' + this.difficultyLevel;
                this.gameSpeed *= 1.2;
                
                // Increase obstacle and fire circle speeds
                this.obstacles.forEach(function(obstacle) {
                    obstacle.body.velocity.x *= 1.2;
                });
                
                this.fireCirclesLeft.forEach(function(circle) {
                    circle.body.velocity.x *= 1.2;
                });
                
                this.fireCirclesRight.forEach(function(circle) {
                    circle.body.velocity.x *= 1.2;
                });
            }
        },
